...
const StatusBorder = ({
  ...
  return (
    <>
      <div
        className={cn(
          "absolute -top-0.5 -left-0.5 h-[calc(100%+4px)] w-[calc(100%+4px)] rounded-[7px] border-3",
          className,
        )}
      />
      {children}
...
export const NodeStatusIndicator = ({
  ...
  switch (status) {
    ...
    case "success":
      return (
        <StatusBorder className={cn("border-green-700", className)}>{children}</StatusBorder>
      );
    case "error":
      return <StatusBorder className={cn("border-red-700", className)}>{children}</StatusBorder>;
    default:
      return <>{children}</>;
  }
};
